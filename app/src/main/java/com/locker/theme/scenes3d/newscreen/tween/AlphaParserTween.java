...
public class AlphaParserTween extends BaseParserTween {
    public BaseTween build() {
        BaseTween baseTween = null;
        if (fromTo.equals("to"))
            baseTween = Tween.to(target, type, duration).target(ScriptUtils.getValue(targetValues[0], Float.class)).delay(delay).ease(equation);
        else
            baseTween = Tween.from(target, type, duration).target(ScriptUtils.getValue(targetValues[0], Float.class)).delay(delay).ease(equation);
